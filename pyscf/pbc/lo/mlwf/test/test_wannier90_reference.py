...
'''
End-to-end validation of pyscf.pbc.lo.mlwf against a compiled wannier90.x.
...
Workflow:
    ...
    4. Build M_mn, A_mn, and band energies with pyscf.pbc.lo.mlwf and write them in
       wannier90 format (.mmn, .amn, .eig).
    5. Run wannier90.x to localize.
    6. Parse final spreads/centers from .wout.
    7. Run pyscf.pbc.lo.mlwf's wannierise on the SAME (M, A, bvectors) inputs.
...
def _write_mmn(path, M_canonical, raw_kpb_idx, raw_kpb_g, perm):
    ...
    with open(path, 'w') as f:
        f.write('Generated by pyscf.pbc.lo.mlwf test harness\n')
