...
RUN_PERFORMANCE_TESTS = getattr(sys.modules.get('__main__'), 'RUN_PERFORMANCE_TESTS', False)
...
class TestISdfxEnergyComparison(unittest.TestCase):
    ...
    def test_rhf_h2_energy(self):
        ...
        self.assertLess(diff, self.ENERGY_TOLERANCE, 
    ...
    def test_rks_lda_energy(self):
        ...
        e_isdfx, e_fftdf, diff = self._compare_energies(
            self.cell_lih, scf.RKS, 
            scf_kwargs={'xc': 'lda,vwn'}
        )
    ...
    def test_rhf_different_isdf_thresholds(self):
        ...
        for thresh in thresholds:
            with self.subTest(threshold=thresh):
                e_isdfx, e_fftdf, diff = self._compare_energies(
                    self.cell_h2, scf.RHF,
                    isdf_kwargs={'isdf_thresh': thresh}
                )
    ...
    def test_rhf_kpoints_diamond(self):
        ...
        mf_isdfx.with_df = ISDFX(mf_isdfx, kmesh=[2, 2, 1])
    ...
    def test_asymmetric_kpoints_123(self):
        ...
        kmesh = [1, 2, 3]
        kpts = self.cell_diamond.make_kpts(kmesh, space_group_symmetry=False, 
                                         time_reversal_symmetry=False, wrap_around=True)
        ...
        from pyscf.occri.isdfx import ISDFX
        mf_isdfx = scf.KRHF(self.cell_diamond, kpts=kpts)
        mf_isdfx.with_df = ISDFX(mf_isdfx, kmesh=kmesh)
    ...
    def test_asymmetric_kpoints_135(self):
        ...
        kmesh = [1, 3, 5]
        kpts = self.cell_h2.make_kpts(kmesh, space_group_symmetry=False,
                                    time_reversal_symmetry=False, wrap_around=True)
    ...
    def test_asymmetric_kpoints_uks(self):
        ...
        kmesh = [2, 1, 3]
        kpts = cell_test.make_kpts(kmesh, space_group_symmetry=False,
                                 time_reversal_symmetry=False, wrap_around=True)
    ...
    @unittest.skipUnless(RUN_PERFORMANCE_TESTS, "Use --perf to run expensive k-point tests")
    def test_prime_kpoints_247(self):
        ...
        kpts = cell.make_kpts(kmesh, space_group_symmetry=False,
                                  time_reversal_symmetry=False, wrap_around=True)
    ...
    def test_metallic_system_edge_case(self):
        ...
        self.assertLess(diff, metal_tolerance,
    ...
    def test_convergence_vs_isdf_threshold(self):
        ...
        for i in range(len(diffs)-1):
            ...
            if diffs[i] * 2.0 < 1.e-12:
                continue
            self.assertLess(diffs[i+1], diffs[i] * 2.0,
