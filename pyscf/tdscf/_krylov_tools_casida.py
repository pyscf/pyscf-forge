...
def ABBA_krylov_solver(matrix_vector_product, hdiag, problem_type='eigenvalue',
    ...
    sub_sigma_p_pi_holder = np.empty_like(sub_a_p_b_holder)
    ...
    log.info(get_mem_info('after put initial guess into V_p_W_holder and V_m_W_holder'))
    ...
    for ii in range(max_iter):
        ...
        log.info(get_mem_info('     MVP stored in holder'))
        ...
        log.info(get_mem_info('     after subgen'))
        ...
        log.info(get_mem_info('     after sub2full compute residual'))
        ...
        r_norms = np.sqrt((residual_1**2).sum(axis=1) + (residual_2**2).sum(axis=1))
