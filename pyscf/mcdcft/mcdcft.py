...
def kernel(mc, dcxc, natorb, occ, root=-1):
    ''' Calculate MC-DCFT total energy
        ...
        Args:
            ...
            dcxc : an instance of density coherence functional class - see dcfnal.py
    ...
    e_tot, E_dc = dcft_energy(dcxc, e_nuc, e_1e, e_coul, e_ncwfn, dm1s, natorb, occ)

    chkdata = {'e_nuc':e_nuc, 'e_1e':e_1e, 'e_coul':e_coul, 'e_ncwfn':e_ncwfn, 'dm':dm1s, 'spin':spin}

    return e_tot, E_dc, chkdata

def dcft_energy(dcxc, e_nuc, e_1e, e_coul, e_ncwfn, dm1s, natorb, occ):
    t0 = (logger.process_clock(), logger.perf_counter())
    ...
    logger.note(dcxc, 'MC-DCFT E = %s, Edc(%s) = %s', e_tot, dcxc.xc_code, E_dc)
    return e_tot, E_dc
...
def _recalculate_with_xc(dcxc, chkdata):
    ...
    return dcft_energy(dcxc, e_nuc, e_1e, e_coul, e_ncwfn, dm1s, natorb, occ)


def get_E_dc(dcxc, oneCDMs, natorb, occ, max_memory=20000, hermi=1):
    ''' E_MCDCFT = h_pq l_pq + 1/2 v_pqrs l_pq l_rs + E_dc[D]
        or, in other terms,
        E_MCDCFT = T_KS[rho] + E_ext[rho] + E_coul[rho] + E_dc[D]
                 = E_DFT[1rdm] - E_xc[rho] + E_dc[D]
        Args:
...
class DCFT_base(lib.StreamObject):
    def __init__(self, mol, dc, xc_preset=None, grids_level=None):
        self.dcfnal = dcfnal(mol, dc, xc_preset, grids_level)
        self.grids_level = grids_level

    def load_mcdcft_chk(self, chkfile):
        self.chkdata = lib.chkfile.load(chkfile, 'mcdcft')

    def recalculate_with_xc(self, dc, xc_preset=None, chkdata=None,
        ...
        ''' Recalculate MC-DCFT total energy based on intermediate quantities from a previous MC-DCFT calculation
            ...
            Args:
                dc : str or dcfnal instance representing the density coherence functional.
                     ...
                     If a dcfnal instance is supplied, `xc_preset` option is ignored.
                xc_preset : dict with detailed parameters to be passed to dcfnal
                chkdata : chkdata dict generated by previous calculation
        ...
        if isinstance(dc, str):
            self.dcfnal = dcfnal(self.mol, dc, xc_preset, grids_level, self.verbose)
        else:
            self.dcfnal = dc
    ...
    def dump_flags(self, verbose=None):
        ...
        log.info('density coherence exchange-correlation functional: %s', self.dcfnal.xc_code)
...
def CASSCFDCFT(mf_or_mol, dc, ncas, nelecas, xc_preset=None, grids_level=None,
    ...
    mc = get_mcdcft_child_class(mc, dc, ncas, nelecas, xc_preset=xc_preset,
