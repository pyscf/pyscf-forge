...
class _ULNO_DF_ERIs:
    ...
    def xform_occ(self, u, spin='a'):
        ...
        for iblk, (i0, i1) in enumerate(lib.prange(0, nocc, occblksize)):
            if iblk == 0:
                ovL[:] = lib.einsum('iax,iI->Iax', self.get_occ_blk(i0, i1, spin=spin), u[i0:i1].conj())
            else:
                ovL[:] += lib.einsum('iax,iI->Iax', self.get_occ_blk(i0, i1, spin=spin), u[i0:i1].conj())
        return ovL
    ...
    def xform_vir(self, u, spin='a'):
        ...
        for i0, i1 in lib.prange(0, nocc, occblksize):
            ovL[i0:i1] = lib.einsum('iax,aA->iAx', self.get_occ_blk(i0, i1, spin=spin), u)
        return ovL
    ...
    def build(self):
        ...
        if self._ovL is None:
            if isinstance(self._ovL_to_save, str):
                ...
                log.info('ovL (alpha/beta) is saved to tmpfile %s', self.feri.filename)
            ...
            raise RuntimeError("Invalid _ovL input. Expecting None or HDF5 file path.")
...
def _init_ump_df_eris(with_df, occ_coeff_a, vir_coeff_a, occ_coeff_b, vir_coeff_b,
    ...
    if isinstance(ovL_a, np.ndarray) and isinstance(ovL_b, np.ndarray):
        ...
        log.debug('aux blksize for incore ao2mo (unrestricted): %d/%d', aux_blksize, naux)
        ...
        for Lpq in loop_df(aux_blksize):
            ...
            ovL_a[:, :, p0:p1] = out_a.reshape(-1, nocca, nvira).transpose(1, 2, 0)
            ovL_b[:, :, p0:p1] = out_b.reshape(-1, noccb, nvirb).transpose(1, 2, 0)
    ...
    # --- Out-of-core (HDF5) ERI generation ---
        ...
        occ_blksize_a = min(nocca, max(1, int(np.floor(mem_avail * 0.6 / mem_occblk_a))))
        if DEBUG_BLKSIZE: occ_blksize_a = max(1, nocca // 2)
        mem_auxblk_a = (occ_blksize_a * nvira + nao_pair) * dsize / 1e6
        aux_blksize_a = min(naux, max(1, int(np.floor(mem_avail * 0.3 / mem_auxblk_a))))
        if DEBUG_BLKSIZE: aux_blksize_a = max(1, naux // 2)
        log.debug('occ blksize (alpha) for outcore ao2mo: %d/%d', occ_blksize_a, nocca)
        log.debug('aux blksize (alpha) for outcore ao2mo: %d/%d', aux_blksize_a, naux)
        ...
        for i0, i1 in lib.prange(0, nocca, occ_blksize_a):
            ...
            p1 = 0
            ovL_block = np.ndarray((nocci, nvira, naux), dtype=dtype, buffer=buf_a)
            for Lpq in loop_df(aux_blksize_a):
                p0, p1 = p1, p1 + Lpq.shape[0]
                out = ao2mo_df(Lpq, mo_a, ijslice, buf2_a)
                ovL_block[:, :, p0:p1] = out.reshape(-1, nocci, nvira).transpose(1, 2, 0)
                Lpq = out = None
        ...
        occ_blksize_b = min(noccb, max(1, int(np.floor(mem_avail * 0.6 / mem_occblk_b))))
        if DEBUG_BLKSIZE: occ_blksize_b = max(1, noccb // 2)
        mem_auxblk_b = (occ_blksize_b * nvirb + nao_pair) * dsize / 1e6
        aux_blksize_b = min(naux, max(1, int(np.floor(mem_avail * 0.3 / mem_auxblk_b))))
        if DEBUG_BLKSIZE: aux_blksize_b = max(1, naux // 2)
        log.debug('occ blksize (beta) for outcore ao2mo: %d/%d', occ_blksize_b, noccb)
        log.debug('aux blksize (beta) for outcore ao2mo: %d/%d', aux_blksize_b, naux)
        ...
        for i0, i1 in lib.prange(0, noccb, occ_blksize_b):
            ...
            p1 = 0
            OVL_block = np.ndarray((nocci, nvirb, naux), dtype=dtype, buffer=buf_b)
            for Lpq in loop_df(aux_blksize_b):
                p0, p1 = p1, p1 + Lpq.shape[0]
                out = ao2mo_df(Lpq, mo_b, ijslice, buf2_b)
                OVL_block[:, :, p0:p1] = out.reshape(-1, nocci, nvirb).transpose(1, 2, 0)
                Lpq = out = None
...
class ULNO(lno.LNO):
    def ao2mo(self, mo_coeff=None):
        ...
        if getattr(self, 'with_df', None):
            ...
            log.debug('ao2mo (non-DF) est mem= %.2f MB  avail mem= %.2f MB', mem_incore, mem_now)
            if ((self._scf._eri is not None or
                ...
                raise NotImplementedError("Unrestricted non-DF out-of-core ERIs not implemented.")
