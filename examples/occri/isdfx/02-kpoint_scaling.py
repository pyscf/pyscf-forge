...
for i in range(len(nk_array)):
    ...
    print(f"{nk_array[i]:<6} {results['time_isdfx'][i]:<10.3f} {results['time_fftdf'][i]:<10.3f} {results['time_build'][i]:<10.3f} {speedup:<10.2f}x")
...
if len(nk_scaling) >= 3:
    ...
    isdfx_r2 = 1 - np.sum((log_time_isdfx - isdfx_predicted)**2) / np.sum((log_time_isdfx - np.mean(log_time_isdfx))**2)
    fftdf_r2 = 1 - np.sum((log_time_fftdf - fftdf_predicted)**2) / np.sum((log_time_fftdf - np.mean(log_time_fftdf))**2)
    build_r2 = 1 - np.sum((log_time_build - build_predicted)**2) / np.sum((log_time_build - np.mean(log_time_build))**2)
    
    print(f"Scaling analysis from log-log fit:")
    ...
    def interpret_scaling(exponent):
        if abs(exponent - 1.0) < 0.1:
            return f"Linear (≈ O(Nk))"
        elif abs(exponent - 2.0) < 0.1:
            return f"Quadratic (≈ O(Nk²))"
    ...
    if isdfx_scaling < fftdf_scaling:
        ...
        print(f"    ISDFX may have higher overhead or need larger Nk to show advantage")
    else:
        print(f"  ≈ Both methods have similar scaling")
